...
import TempoEntregaTab from './TempoEntregaTab';
...
export default function AbastecimentoFarmaceutico({ onNavigateToTV, user }: AbastecimentoFarmaceuticoProps) {
  const [activeSubTab, setActiveSubTab] = useState<'geral' | 'tempo_entrega'>('geral');
...
<div class="footer">
  ...
  // ── TELA DE UPLOAD ──
  if (activeSubTab === 'tempo_entrega') {
    return (
      <div className="flex flex-col h-full">
        <div className="bg-white border-b border-slate-200 px-6 py-2 flex gap-4">
          <button 
            onClick={() => setActiveSubTab('geral')}
            className={`px-4 py-2 text-sm font-semibold border-b-2 transition-colors ${activeSubTab === 'geral' ? 'border-violet-600 text-violet-600' : 'border-transparent text-slate-500 hover:text-slate-700'}`}
          >
            Visão Geral
          </button>
          <button 
            onClick={() => setActiveSubTab('tempo_entrega')}
            className={`px-4 py-2 text-sm font-semibold border-b-2 transition-colors ${activeSubTab === 'tempo_entrega' ? 'border-violet-600 text-violet-600' : 'border-transparent text-slate-500 hover:text-slate-700'}`}
          >
            Análise de Tempo de Entrega
          </button>
        </div>
        <div className="flex-1 overflow-hidden">
          <TempoEntregaTab user={user} />
        </div>
      </div>
    );
  }
